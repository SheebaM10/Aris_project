              ...
              <div className="space-y-4">
                {[
                  { 
                    ...
                    status: 'Profiles Sent', 
                    employees: ['Griffith Sheeba Menon', 'Shivani B T'], 
                  ...
                  { 
                    ...
                    employees: ['Athulya Roy'], 
                    lastUpdate: '2025-09-01',
                    nextAction: 'Training completion: Sept 15'
                  },
                  { 
                    client: 'DataCorp', 
                    project: 'Analytics Dashboard', 
                    status: 'Interview Scheduled', 
                    employees: ['Tom Wilson', 'Sarah Park'], 
                    lastUpdate: '2025-08-30',
                    nextAction: 'Interviews: Sept 5'
                  }
                ].map((project, idx) => (
                  <div key={idx} className="border rounded-lg p-4">
                    <div className="flex items-center justify-between mb-3">
                    ...
                    <div className="grid grid-cols-2 gap-4 mb-3">
                      <div>
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {project.employees.map((emp, empIdx) => (
                            <Badge key={empIdx} variant="secondary" className="text-xs">
                              {emp}
                            </Badge>
                          ))}
                        </div>
                      ...
                      <div>
                        ...
                        <p className="font-medium text-sm">{project.nextAction}</p>
                      </div>
                    </div>
                    <div className="flex gap-2">
                      <Button 
                        ...
                        variant="outline"
                        onClick={async () => {
                          const clientEmail = project.client === 'TechCorp Solutions' ? 'contact@techcorp.com' :
                                               project.client === 'InnovateInc' ? 'hiring@innovateinc.com' : 'hr@datacorp.com'
                          
                          const success = await sendEmail(
                            clientEmail,
                            `Update on ${project.project}`,
                            `Dear ${project.client} team,\n\nWe have an update on your project "${project.project}".\n\nCurrent Status: ${project.status}\nNext Action: ${project.nextAction}\n\nPlease let us know if you have any questions.\n\nBest regards,\nHR Team - ARIS`,
                            'general',
                            { clientName: project.client, projectName: project.project }
                          )
                          
                          if (success) {
                            alert(`Email sent successfully to ${project.client}!`)
                          } else {
                      ...
                      <Button 
                        ...
                        onClick={async () => {
                          ...
                          const success = await sendEmail(
                            clientEmail,
                            `Interview Scheduling for ${project.project}`,
                            `Dear ${project.client} team,\n\nWe would like to schedule interviews for the candidates we've proposed for your project "${project.project}".\n\nProposed candidates: ${project.employees.join(', ')}\n\nPlease let us know your availability for the coming week.\n\nBest regards,\nHR Team - ARIS`,
                            'interview_scheduled',
                            { clientName: project.client, projectName: project.project, candidates: project.employees }
                          )
                          
                          if (success) {
                            alert(`Interview scheduling email sent to ${project.client}!`)
                          } else {
                            alert('Failed to send email. Please try again.')
                          }
                        }}
