// Employee Login Page (merged from solo-view-main Auth)
'use client';
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#f9fafb' }}>
      <form onSubmit={isSignUp ? handleSignUp : handleLogin} style={{ background: 'white', padding: '2rem 2.5rem', borderRadius: '1rem', boxShadow: '0 2px 16px rgba(0,0,0,0.08)', minWidth: 340 }}>
        <h2 style={{ fontSize: '1.3rem', fontWeight: 600, marginBottom: '1.5rem', textAlign: 'center' }}>{isSignUp ? 'Employee Sign Up' : 'Employee Login'}</h2>
        ...
        <div style={{ marginTop: '1rem', textAlign: 'center' }}>
          {isSignUp ? (
            <span>Already have an account?{' '}
              <button type="button" onClick={() => { setIsSignUp(false); setError(''); }} style={{ color: '#2563eb', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Log In</button>
            </span>
          ) : (
            <span>Don&apos;t have an account?{' '}
              <button type="button" onClick={() => { setIsSignUp(true); setError(''); }} style={{ color: '#059669', background: 'none', border: 'none', cursor: 'pointer', textDecoration: 'underline' }}>Sign Up</button>
            </span>
          )}
        </div>
      </form>
